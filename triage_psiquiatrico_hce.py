...
# --------------------------------------------------------------
# El PIN se lee de la variable de entorno APP_PIN si existe;
# si no, usa "1234" por defecto (CÁMBIALO antes de compartir la URL).
#
...
with st.sidebar.expander("➕ Agregar nuevo paciente", expanded=False):
    with st.form("form_nuevo_paciente", clear_on_submit=True):
        ...
        nivel_riesgo_form = st.selectbox(
            ...
            index=2,
            help="Determina el color/etiqueta y sugiere el rango de CIE típico.",
        )
        cie_sugeridos = {
            "Urgente": ["F32.3", "F31.5"],
            "Medio-Alto": ["F32.2", "F33.2"],
            "Medio": ["F32.1", "F33.1"],
            "Bajo": ["F32.0", "F33.4"],
        }
        cie_form = st.selectbox("Código CIE-10", options=cie_sugeridos[nivel_riesgo_form])
        diagnostico_form = st.text_input(
            "Diagnóstico formal",
            placeholder="Ej: Episodio depresivo moderado (CIE-10 F32.1)",
        )
