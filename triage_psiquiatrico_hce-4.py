...
# --------------------------------------------------------------
# ESTADO DE SESIÓN: lista de pacientes editable (permite agregar nuevos)
# --------------------------------------------------------------
if "pacientes" not in st.session_state:
    st.session_state.pacientes = copy.deepcopy(PACIENTES)


def renumerar_pacientes():
    """Reordena la lista por prioridad y renumera 1..N de forma secuencial."""
    st.session_state.pacientes.sort(key=lambda x: x["prioridad"])
    for i, p in enumerate(st.session_state.pacientes, start=1):
        p["prioridad"] = i


def agregar_paciente(nuevo_paciente: dict, posicion_deseada: int):
    """Inserta un paciente nuevo en la posición deseada y renumera la lista."""
    lista = st.session_state.pacientes
    posicion_deseada = max(1, min(posicion_deseada, len(lista) + 1))
    # Desplazamos +1 la prioridad de todos los pacientes desde la posición deseada en adelante
    for p in lista:
...
def actualizar_paciente(paciente_obj: dict, datos_actualizados: dict, nueva_prioridad: int):
    """Actualiza los datos de un paciente existente (por referencia al dict) y,
    si cambió la prioridad, lo reubica en la posición correspondiente."""
    lista = st.session_state.pacientes
    paciente_obj.update(datos_actualizados)

    if nueva_prioridad != paciente_obj["prioridad"]:
        vieja_prioridad = paciente_obj["prioridad"]
        lista.remove(paciente_obj)
        # Cerramos el hueco que dejó en su posición anterior
        for p in lista:
            if p["prioridad"] > vieja_prioridad:
                p["prioridad"] -= 1
        # Abrimos espacio en la nueva posición deseada
        nueva_prioridad = max(1, min(nueva_prioridad, len(lista) + 1))
        for p in lista:
            if p["prioridad"] >= nueva_prioridad:
                p["prioridad"] += 1
        paciente_obj["prioridad"] = nueva_prioridad
        lista.append(paciente_obj)

    renumerar_pacientes()


def eliminar_paciente(paciente_obj: dict):
    """Elimina un paciente de la lista y renumera las prioridades restantes."""
    lista = st.session_state.pacientes
    if paciente_obj in lista:
        lista.remove(paciente_obj)
    renumerar_pacientes()
    ...
    with st.form("form_nuevo_paciente", clear_on_submit=True):
        ...
        if enviado:
            if not nombre_form.strip():
                ...
                agregar_paciente(nuevo, int(posicion_form))
                # Recalculamos dónde quedó el paciente recién agregado para dejarlo seleccionado
                st.session_state.pacientes.sort(key=lambda x: x["prioridad"])
                nuevo_indice = next(
                    i for i, p in enumerate(st.session_state.pacientes) if p is nuevo
                )
                st.session_state["indice_seleccion_pendiente"] = nuevo_indice
                st.success(f"Paciente agregado en la posición #{nuevo['prioridad']}.")
                st.rerun()
    ...
    with st.form(clave_form):
        ...
        if guardar:
            if not nombre_edit.strip():
                ...
                nuevo_indice = next(
                    i for i, p in enumerate(st.session_state.pacientes) if p is paciente
                )
                st.session_state["indice_seleccion_pendiente"] = nuevo_indice
